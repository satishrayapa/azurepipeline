...
IF EXISTS (select TOP 1 1 from sys.tables where Name = 'txdCNHandbookPCDetail' 
			AND Type = 'U')
BEGIN	
...
BEGIN
	...
	CREATE TABLE [dbo].[txdCNHandbookPCDetail]  
	(
		PartnerId INT NOT NULL,
		EffDate DATETIME NOT NULL,
		CNHandbookHeaderGUID VARCHAR(50) NOT NULL,
		CNHandbookPCDetailGUID VARCHAR(50) NOT NULL,
		...
		StorageTime NVARCHAR(8),
		DeletedFlag VARCHAR(1) NOT NULL,
		KeepDuringRollback VARCHAR(1) NOT NULL
	) ON [PRIMARY]


		ALTER TABLE [dbo].[txdCNHandbookPCDetail] ADD CONSTRAINT [PK_txdCNHandbookPCDetail] PRIMARY KEY NONCLUSTERED 
		(
			PartnerId ASC,
			CNHandbookHeaderGUID ASC,
			CNHandbookPCDetailGUID ASC
		)
END

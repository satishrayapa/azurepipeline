...
----------------------------------------------------------------------------------------------------------------
PRINT '.........CREATE/MODIFY tmdDPSSearchSettings.............'
IF EXISTS (SELECT * FROM dbo.syscolumns WHERE name = 'PreFilterValue' AND ID = OBJECT_ID('tmdDPSSearchSettings'))
	BEGIN
		PRINT 'Column Already Exists... Skipping'
	END
ELSE
	BEGIN
		if exists (select * from dbo.sysobjects where name = 'PK_tmdDPSSearchSettings') 
			begin
				ALTER TABLE [dbo].[tmdDPSSearchSettings] DROP CONSTRAINT [PK_tmdDPSSearchSettings]
			end

		if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[tmp_tmdDPSSearchSettings]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
			begin
				DROP TABLE tmp_tmdDPSSearchSettings
			end
		
		CREATE TABLE [dbo].[Tmp_tmdDPSSearchSettings]
			(
		...
		if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[tmdDPSSearchSettings]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
			begin
				PRINT 'INSERT INTO TEMP TABLE......'
				 EXEC('INSERT INTO [dbo].[tmp_tmdDPSSearchSettings]
           ([PartnerID]
           ,[EffDate]
           ,[SettingsDescription]
		...
		if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[tmdDPSSearchSettings]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
			BEGIN
				PRINT ''
				PRINT 'DROP TABLE..........'
				DROP TABLE [dbo].[tmdDPSSearchSettings]
			END
					
		PRINT ''
		PRINT 'RENAME TABLE....'
		PRINT ''
		EXECUTE sp_rename N'dbo.Tmp_tmdDPSSearchSettings', N'tmdDPSSearchSettings', 'OBJECT'
				
				
		PRINT ''
		PRINT 'CREATE PRIMARY KEY......'
		ALTER TABLE [dbo].[tmdDPSSearchSettings] ADD CONSTRAINT
			PK_tmdDPSSearchSettings PRIMARY KEY NONCLUSTERED 
			(
				[PartnerId] ASC,
				[SettingsID] ASC
			) WITH FILLFACTOR = 90 ON [PRIMARY]

		CREATE INDEX CIX_tmdDPSSearchSettings on tmdDPSSearchSettings (EffDate asc)

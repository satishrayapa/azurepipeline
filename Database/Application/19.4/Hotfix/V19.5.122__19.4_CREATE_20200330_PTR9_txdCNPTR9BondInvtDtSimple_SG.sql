...
IF EXISTS (select TOP 1 1 from sys.tables where Name = 'txdCNPTR9BondInvtDtSimple'
			AND Type = 'U')
BEGIN	
...
BEGIN
	...
	CREATE TABLE [dbo].[txdCNPTR9BondInvtDtSimple]
	(
		...
		BondInvtDtSimpleGUID VARCHAR(50) 	NOT NULL,	
		...
		ALTER TABLE [dbo].[txdCNPTR9BondInvtDtSimple] ADD CONSTRAINT [PK_txdCNPTR9BondInvtDtSimple] PRIMARY KEY NONCLUSTERED
		(
			PartnerId ASC,
			DeclarationGUID ASC,
			DeclarationDataGUID ASC,
			ResponseGUID ASC,
			BondInvtDtSimpleGUID ASC
		)
END

...
IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'MXTariffNum' --your column here
			AND Object_ID = OBJECT_ID('txdMXSaaiComplimentaryImports') --Your Table Here
...
BEGIN
	...
	EXEC usp_DBACopyTableIndexesByColumn '','txdMXSaaiComplimentaryImports','MXTariffNum','varchar',1,10

	ALTER TABLE txdMXSaaiComplimentaryImports --Your Table Here
	ALTER COLUMN MXTariffNum [varchar] (10) NOT NULL --your column here

	--Do not change 1st paramter.  
	EXEC usp_DBACreateTableIndexes '','txdMXSaaiComplimentaryImports' --Your Table Here

	-- Format leading zeros
	UPDATE txdMXSaaiComplimentaryImports SET MXTariffNum = REPLACE(STR(MXTariffNum,8),' ','0')
END

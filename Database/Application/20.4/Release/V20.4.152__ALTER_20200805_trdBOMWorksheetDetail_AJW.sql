...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'trdBOMWorksheetDetail' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'AddlIndicators' --Any 1 of your NEW columns here
	...
	BEGIN
		IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'DeletedFlag' --Leave DeletedFlag here
				AND Object_ID = Object_ID('trdBOMWorksheetDetail')) --Your Table Here
		BEGIN
			SELECT @NumRows = COUNT(*) FROM dbo.trdBOMWorksheetDetail WITH (NOLOCK) --Your Table Here
		...
		BEGIN
			WHILE 1 = 1
			BEGIN
				SELECT TOP 1 @sql = N'alter table [trdBOMWorksheetDetail] ' --Your Table Here
						+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('trdBOMWorksheetDetail') --Your Table Here
			...
			exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'trdBOMWorksheetDetail', @Columns = @columnchanges, @ForceCopy = 0 --Your Table Here
			
			ALTER TABLE dbo.trdBOMWorksheetDetail --Your Table Here
			DROP COLUMN DeletedFlag, KeepDuringRollback
										
			ALTER TABLE dbo.trdBOMWorksheetDetail --Your Table Here
			ADD  
			AddlIndicators VARCHAR(50) NOT NULL DEFAULT '' --your NEW columns here
			...
			BEGIN
				SELECT TOP 1
				@sql = N'alter table [trdBOMWorksheetDetail] ' --Your Table Here
					+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('trdBOMWorksheetDetail') --Your Table Here
					AND [name] IN ( 'AddlIndicators') --your NEW columns here
					AND default_object_id <> 0

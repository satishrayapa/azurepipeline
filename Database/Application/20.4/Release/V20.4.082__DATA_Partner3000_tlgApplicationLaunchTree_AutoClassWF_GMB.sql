...
BEGIN
	IF EXISTS (
			SELECT TOP 1 1
			FROM sys.tables
			WHERE Name = 'tlgApplicationLaunchTree' --Your Table Here
				AND Type = 'U'
			)
	BEGIN
		DECLARE @NotificationSequenceNum AS INT = (SELECT SequenceNo FROM tlgApplicationLaunchTree WHERE PartnerID = @baselinePartner AND WorkFlow = 'STANDARD GLOBAL CLASSIFICATION INTERFACE' AND Command = 'WORKFLOW NOTIFICATION')

		update tlgApplicationLaunchTree
		set SequenceNo = @NotificationSequenceNum + 1 WHERE PartnerID = @baselinePartner AND WorkFlow = 'STANDARD GLOBAL CLASSIFICATION INTERFACE' AND Command = 'WORKFLOW NOTIFICATION'
		
		insert into tlgApplicationLaunchTree
		select @baselinePartner, GETDATE(), 'STANDARD GLOBAL CLASSIFICATION INTERFACE', @NotificationSequenceNum, 'dxdGlobalClassificationWF.dll', 'SMART HS AUTO CLASSIFICATION', 'N', 'N'
	END
END
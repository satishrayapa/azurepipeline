...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'tmdLicenseHeader' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'RegulationType' --Any 1 of your NEW columns here
				AND Object_ID = Object_ID('tmdLicenseHeader')) --Your Table Here
	BEGIN
	...
	BEGIN
		IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'DeletedFlag' --Leave DeletedFlag here
				AND Object_ID = Object_ID('tmdLicenseHeader')) --Your Table Here
		BEGIN
			SELECT @NumRows = COUNT(*) FROM dbo.tmdLicenseHeader WITH (NOLOCK) --Your Table Here
		...
		BEGIN
			WHILE 1 = 1
			BEGIN
				SELECT TOP 1 @sql = N'alter table [tmdLicenseHeader] ' --Your Table Here
						+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('tmdLicenseHeader') --Your Table Here
			...
			exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'tmdLicenseHeader', @Columns = @columnchanges, @ForceCopy = 0 --Your Table Here
			
			ALTER TABLE dbo.tmdLicenseHeader --Your Table Here
			DROP COLUMN DeletedFlag, KeepDuringRollback
										
			ALTER TABLE dbo.tmdLicenseHeader --Your Table Here
			ADD  
			RegulationType NVARCHAR(30) NOT NULL DEFAULT '' --your NEW columns here
			...
			BEGIN
				SELECT TOP 1
				@sql = N'alter table [tmdLicenseHeader] ' --Your Table Here
					+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('tmdLicenseHeader') --Your Table Here
					AND [name] IN ( 'RegulationType') --your NEW columns here
					AND default_object_id <> 0
			...
			EXEC usp_DBACreateTableIndexes '','tmdLicenseHeader' --Your Table Here
		END
	END
END
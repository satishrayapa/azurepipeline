...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'txdFTACertRequestHeader' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'Notes' --Any 1 of your NEW columns here
	...
	BEGIN
		IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'DeletedFlag' --Leave DeletedFlag here
				AND Object_ID = Object_ID('txdFTACertRequestHeader')) --Your Table Here
		BEGIN
			SELECT @NumRows = COUNT(*) FROM dbo.txdFTACertRequestHeader WITH (NOLOCK) --Your Table Here
		...
		BEGIN
			WHILE 1 = 1
			BEGIN
				SELECT TOP 1 @sql = N'alter table [txdFTACertRequestHeader] ' --Your Table Here
						+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdFTACertRequestHeader') --Your Table Here
			...
			exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'txdFTACertRequestHeader', @Columns = @columnchanges, @ForceCopy = 0 --Your Table Here
			
			ALTER TABLE dbo.txdFTACertRequestHeader --Your Table Here
			DROP COLUMN DeletedFlag, KeepDuringRollback
										
			ALTER TABLE dbo.txdFTACertRequestHeader --Your Table Here
			ADD  
			Notes NVARCHAR(4000) NOT NULL DEFAULT '' --your NEW columns here
			,DueDate [DateTime] NOT NULL DEFAULT '1/1/1900'
			...
			BEGIN
				SELECT TOP 1
				@sql = N'alter table [txdFTACertRequestHeader] ' --Your Table Here
					+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdFTACertRequestHeader') --Your Table Here
					AND [name] IN ( 'Notes', 'DueDate' ) --your NEW columns here
					AND default_object_id <> 0

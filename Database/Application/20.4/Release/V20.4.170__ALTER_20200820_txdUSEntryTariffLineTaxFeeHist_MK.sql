...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'txdUSEntryTariffLineTaxFeeHist' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'SpecificRt' --Any 1 of your NEW columns here
				AND Object_ID = Object_ID('txdUSEntryTariffLineTaxFeeHist')) --Your Table Here
	BEGIN
	...
	BEGIN
		IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'DeletedFlag' --Leave DeletedFlag here
				AND Object_ID = Object_ID('txdUSEntryTariffLineTaxFeeHist')) --Your Table Here
		BEGIN
			SELECT @NumRows = COUNT(*) FROM dbo.txdUSEntryTariffLineTaxFeeHist WITH (NOLOCK) --Your Table Here
		...
		BEGIN
			WHILE 1 = 1
			BEGIN
				SELECT TOP 1 @sql = N'alter table [txdUSEntryTariffLineTaxFeeHist] ' --Your Table Here
						+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdUSEntryTariffLineTaxFeeHist') --Your Table Here
			...
			exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'txdUSEntryTariffLineTaxFeeHist', @Columns = @columnchanges, @ForceCopy = 0 --Your Table Here
			
			ALTER TABLE dbo.txdUSEntryTariffLineTaxFeeHist --Your Table Here
			DROP COLUMN DeletedFlag, KeepDuringRollback
										
			ALTER TABLE dbo.txdUSEntryTariffLineTaxFeeHist --Your Table Here
			ADD  
			SpecificRt NUMERIC(38,20) NOT NULL DEFAULT 0.0 --your NEW columns here
			,AdvaloremRt NUMERIC(38,20) NOT NULL DEFAULT 0.0 --your NEW columns here
			...
			BEGIN
				SELECT TOP 1
				@sql = N'alter table [txdUSEntryTariffLineTaxFeeHist] ' --Your Table Here
					+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdUSEntryTariffLineTaxFeeHist') --Your Table Here
					AND [name] IN ('SpecificRt','AdvaloremRt') --your NEW columns here
					AND default_object_id <> 0
			...
			EXEC usp_DBACreateTableIndexes '','txdUSEntryTariffLineTaxFeeHist' --Your Table Here
		END
	END
END

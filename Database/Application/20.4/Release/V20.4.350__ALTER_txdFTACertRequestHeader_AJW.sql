...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			...
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'BusinessStatus' --Any 1 of your NEW columns here
	...
	BEGIN
		...
		BEGIN
			...
			ALTER TABLE dbo.txdFTACertRequestHeader --Your Table Here
			ADD  
			BusinessStatus VARCHAR(50) NOT NULL DEFAULT '' --your NEW columns here
			...
			BEGIN
				...
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdFTACertRequestHeader') --Your Table Here
					AND [name] IN ( 'BusinessStatus') --your NEW columns here
					AND default_object_id <> 0

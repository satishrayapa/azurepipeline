...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'txdUSEVGovernmentData' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'HsNum2' --Any 1 of your NEW columns here
				AND Object_ID = Object_ID('txdUSEVGovernmentData')) --Your Table Here
	BEGIN
	...
	BEGIN
		IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'DeletedFlag' --Leave DeletedFlag here
				AND Object_ID = Object_ID('txdUSEVGovernmentData')) --Your Table Here
		BEGIN
			SELECT @NumRows = COUNT(*) FROM dbo.txdUSEVGovernmentData WITH (NOLOCK) --Your Table Here
		...
		BEGIN
			WHILE 1 = 1
			BEGIN
				SELECT TOP 1 @sql = N'alter table [txdUSEVGovernmentData] ' --Your Table Here
						+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdUSEVGovernmentData') --Your Table Here
			...
			exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'txdUSEVGovernmentData', @Columns = @columnchanges, @ForceCopy = 0 --Your Table Here
			
			ALTER TABLE dbo.txdUSEVGovernmentData --Your Table Here
			DROP COLUMN DeletedFlag, KeepDuringRollback
										
			ALTER TABLE dbo.txdUSEVGovernmentData --Your Table Here
			ADD [HsNum2] [varchar] (12) NOT NULL DEFAULT '',
			...
			BEGIN
				SELECT TOP 1
				@sql = N'alter table [txdUSEVGovernmentData] ' --Your Table Here
					+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdUSEVGovernmentData') --Your Table Here
					AND [name] IN ('HsNum2') --your NEW columns here
					AND default_object_id <> 0
			...
			EXEC usp_DBACreateTableIndexes '','txdUSEVGovernmentData' --Your Table Here
		END
	END
END
...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			...
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'UserName' --NEW column here
	...
	BEGIN
		...
		BEGIN
			...
			PRINT 'SET A VALID USER NAME FOR NEW COLUMN............................'
			IF EXISTS (SELECT TOP 1 1 FROM sys.tables
				WHERE Name = 'trdReportMXAnnualMaquila' --Your Table Here
				AND Type = 'U')
				BEGIN
					Declare @UserName varchar(80)
					select @UserName = MAX(UserName) from trdReportMXAnnualMaquila

					if @UserName is not null
						EXEC('UPDATE txdMXAnnualMaquilaReportHeader SET UserName = ''' + @UserName + '''')
				END

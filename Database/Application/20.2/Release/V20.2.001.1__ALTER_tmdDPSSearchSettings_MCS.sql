...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'tmdDPSSearchSettings' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'PreFilterValue' --Any 1 of your NEW columns here
				AND Object_ID = Object_ID('tmdDPSSearchSettings')) --Your Table Here
	BEGIN
	...
	BEGIN
		IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'DeletedFlag' --Leave DeletedFlag here
				AND Object_ID = Object_ID('tmdDPSSearchSettings')) --Your Table Here
		BEGIN
			SELECT @NumRows = COUNT(*) FROM dbo.tmdDPSSearchSettings WITH (NOLOCK) --Your Table Here
		...
		BEGIN
			WHILE 1 = 1
			BEGIN
				SELECT TOP 1 @sql = N'alter table [tmdDPSSearchSettings] ' --Your Table Here
						+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('tmdDPSSearchSettings') --Your Table Here
			...
			exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'tmdDPSSearchSettings', @Columns = @columnchanges, @ForceCopy = 0 --Your Table Here
			
			ALTER TABLE dbo.tmdDPSSearchSettings --Your Table Here
			...
			BEGIN
				SELECT TOP 1
				@sql = N'alter table [tmdDPSSearchSettings] ' --Your Table Here
					+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('tmdDPSSearchSettings') --Your Table Here
			...
			EXEC usp_DBACreateTableIndexes '','tmdDPSSearchSettings' --Your Table Here
		END
	END
END

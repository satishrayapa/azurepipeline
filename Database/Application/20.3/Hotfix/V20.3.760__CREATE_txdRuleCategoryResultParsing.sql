IF EXISTS (select TOP 1 1 from sys.tables where Name = 'txdRuleCategoryResultParsing' --Your Table Here
			AND Type = 'U')
BEGIN	
...
ELSE
BEGIN
	CREATE TABLE [dbo].[txdRuleCategoryResultParsing] --Your Table Here
	(
		...
		CREATE CLUSTERED INDEX [CIX_txdRuleCategoryResultParsing] ON [dbo].[txdRuleCategoryResultParsing]  --Your Table Here
		(
			[EffDate] ASC
		)

		ALTER TABLE [dbo].[txdRuleCategoryResultParsing] ADD CONSTRAINT [PK_txdRuleCategoryResultParsing] PRIMARY KEY NONCLUSTERED --Your Table Here
		(
			[PartnerID] ASC,
			[RuleCategory] ASC,
			[UsedDM] ASC,

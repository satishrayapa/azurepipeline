...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'txdAdjStatusHist' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'Transmitted' --Any 1 of your NEW columns here
				AND Object_ID = Object_ID('txdAdjStatusHist')) --Your Table Here
	BEGIN
	...
	BEGIN
		IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'DeletedFlag' --Leave DeletedFlag here
				AND Object_ID = Object_ID('txdAdjStatusHist')) --Your Table Here
		BEGIN
			SELECT @NumRows = COUNT(*) FROM dbo.txdAdjStatusHist WITH (NOLOCK) --Your Table Here
		...
		BEGIN
			WHILE 1 = 1
			BEGIN
				SELECT TOP 1 @sql = N'alter table [txdAdjStatusHist] ' --Your Table Here
						+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdAdjStatusHist') --Your Table Here
			...
			exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'txdAdjStatusHist', @Columns = @columnchanges, @ForceCopy = 0 --Your Table Here
			
			ALTER TABLE dbo.txdAdjStatusHist --Your Table Here
			DROP COLUMN DeletedFlag, KeepDuringRollback
										
			ALTER TABLE dbo.txdAdjStatusHist --Your Table Here
			ADD  
			Transmitted VARCHAR(1) NOT NULL DEFAULT 'Y' --your NEW columns here
			...
			BEGIN
				SELECT TOP 1
				@sql = N'alter table [txdAdjStatusHist] ' --Your Table Here
					+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdAdjStatusHist') --Your Table Here
					AND [name] IN ( 'Transmitted' ) --your NEW columns here
					AND default_object_id <> 0
			...
			EXEC usp_DBACreateTableIndexes '','txdAdjStatusHist' --Your Table Here
		END
	END
END
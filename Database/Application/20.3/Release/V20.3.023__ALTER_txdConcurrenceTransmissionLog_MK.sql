...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'txdConcurrenceTransmissionLog' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'VersionID' --Any 1 of your NEW columns here
				AND Object_ID = Object_ID('txdConcurrenceTransmissionLog')) --Your Table Here
	BEGIN
	...
	BEGIN
		IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'DeletedFlag' --Leave DeletedFlag here
				AND Object_ID = Object_ID('txdConcurrenceTransmissionLog')) --Your Table Here
		BEGIN
			SELECT @NumRows = COUNT(*) FROM dbo.txdConcurrenceTransmissionLog WITH (NOLOCK) --Your Table Here
		...
		BEGIN
			WHILE 1 = 1
			BEGIN
				SELECT TOP 1 @sql = N'alter table [txdConcurrenceTransmissionLog] ' --Your Table Here
						+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdConcurrenceTransmissionLog') --Your Table Here
			...
			exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'txdConcurrenceTransmissionLog', @Columns = @columnchanges, @ForceCopy = 0 --Your Table Here
			
			ALTER TABLE dbo.txdConcurrenceTransmissionLog --Your Table Here
			DROP COLUMN DeletedFlag, KeepDuringRollback
										
			ALTER TABLE dbo.txdConcurrenceTransmissionLog --Your Table Here
			ADD  
			VersionID uniqueidentifier NOT NULL DEFAULT CAST(0x0 as uniqueidentifier)  --your NEW columns here
			...
			BEGIN
				SELECT TOP 1
				@sql = N'alter table [txdConcurrenceTransmissionLog] ' --Your Table Here
					+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdConcurrenceTransmissionLog') --Your Table Here
					AND [name] IN ( 'VersionID' ) --your NEW columns here
					AND default_object_id <> 0
			...
			EXEC usp_DBACreateTableIndexes '','txdConcurrenceTransmissionLog' --Your Table Here
		END
	END
END
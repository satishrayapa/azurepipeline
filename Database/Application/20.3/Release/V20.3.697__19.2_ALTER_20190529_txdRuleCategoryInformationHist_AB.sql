...
----------------------------------------------------------------------------------------------------------------
PRINT '.........CREATE/MODIFY txdRuleCategoryInformationHist.............'

IF EXISTS (
		SELECT *
		FROM dbo.syscolumns
		WHERE name = 'UsesCountryOfOrigin'
			AND ID = OBJECT_ID('txdRuleCategoryInformationHist')
		)
BEGIN
...
BEGIN
	IF EXISTS (
			SELECT *
			FROM dbo.sysobjects
			WHERE name = 'PK_txdRuleCategoryInformationHist'
			)
	BEGIN
		ALTER TABLE [dbo].[txdRuleCategoryInformationHist]

		DROP CONSTRAINT [PK_txdRuleCategoryInformationHist]
	END

	IF EXISTS (
			SELECT *
			FROM dbo.sysobjects
			WHERE id = object_id(N'[dbo].[tmp_txdRuleCategoryInformationHist]')
				AND OBJECTPROPERTY(id, N'IsUserTable') = 1
			)
	BEGIN
		DROP TABLE tmp_txdRuleCategoryInformationHist
	END

	CREATE TABLE [dbo].[Tmp_txdRuleCategoryInformationHist] (
	...
	IF EXISTS (
			SELECT *
			FROM dbo.sysobjects
			WHERE id = object_id(N'[dbo].[txdRuleCategoryInformationHist]')
				AND OBJECTPROPERTY(id, N'IsUserTable') = 1
			)
	BEGIN
		...
		EXEC (
				'INSERT INTO dbo.Tmp_txdRuleCategoryInformationHist (
							PartnerID
							,EffDate
							,RuleCategory
							,ParentFTA
						...
						SELECT PartnerID
							...
							,DeletedFlag
							,KeepDuringRollback
						FROM dbo.txdRuleCategoryInformationHist TABLOCKX'
				)
	END
	...
	BEGIN
		...
		DROP TABLE [dbo].[txdRuleCategoryInformationHist]
	END
	...
	EXECUTE sp_rename N'dbo.Tmp_txdRuleCategoryInformationHist'
		,N'txdRuleCategoryInformationHist'
		,'OBJECT'
	...
	CREATE CLUSTERED INDEX [CIX_txdRuleCategoryInformationHist] ON [dbo].[txdRuleCategoryInformationHist] ([EffDate] ASC)

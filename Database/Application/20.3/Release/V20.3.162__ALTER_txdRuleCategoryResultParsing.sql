...
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'txdRuleCategoryResultParsing' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'UserName' --Any 1 of your NEW columns here
				AND Object_ID = Object_ID('txdRuleCategoryResultParsing')) --Your Table Here
	BEGIN
	...
	BEGIN
		IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'DeletedFlag' --Leave DeletedFlag here
				AND Object_ID = Object_ID('txdRuleCategoryResultParsing')) --Your Table Here
		BEGIN
			SELECT @NumRows = COUNT(*) FROM dbo.txdRuleCategoryResultParsing WITH (NOLOCK) --Your Table Here
		...
		BEGIN
			WHILE 1 = 1
			BEGIN
				SELECT TOP 1 @sql = N'alter table [txdRuleCategoryResultParsing] ' --Your Table Here
						+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdRuleCategoryResultParsing') --Your Table Here
			...
			exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'txdRuleCategoryResultParsing', @Columns = @columnchanges, @ForceCopy = 0 --Your Table Here
			
			ALTER TABLE dbo.txdRuleCategoryResultParsing --Your Table Here
			DROP COLUMN DeletedFlag, KeepDuringRollback
										
			ALTER TABLE dbo.txdRuleCategoryResultParsing --Your Table Here
			ADD  
			UserName VARCHAR(50) NOT NULL DEFAULT '' --your NEW columns here	
			,LastUpdatedUser VARCHAR(50) NOT NULL DEFAULT ''
			...
			BEGIN
				SELECT TOP 1
				@sql = N'alter table [txdRuleCategoryResultParsing] ' --Your Table Here
					+ 'drop constraint [' + object_name(sc.default_object_id) + N']'
				FROM    sys.columns sc
				WHERE   OBJECT_ID = OBJECT_ID('txdRuleCategoryResultParsing') --Your Table Here
					AND [name] IN ( 'UserName','LastUpdatedUser','LastUpdatedDate' ) --your NEW columns here
					AND default_object_id <> 0
			...
			EXEC usp_DBACreateTableIndexes '','txdRuleCategoryResultParsing' --Your Table Here
		END
	END
END
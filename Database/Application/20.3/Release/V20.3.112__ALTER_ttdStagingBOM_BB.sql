...
IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name in('BillOfMaterials','FGProductNum','FGPlantID','FGNote','CProductNum','CManufacturerID','CNote') --your column here
			AND Object_ID = OBJECT_ID('ttdStagingBOM')) --Your Table Here
BEGIN
	...
	exec usp_DBACopyTableIndexesByMultiColumn NULL,@TableName = 'ttdStagingBOM', @Columns = @columnchanges, @ForceCopy = 0

	ALTER TABLE ttdStagingBOM --Your Table Here
	...
	EXEC usp_DBACreateTableIndexes '','ttdStagingBOM' --Your Table Here
End
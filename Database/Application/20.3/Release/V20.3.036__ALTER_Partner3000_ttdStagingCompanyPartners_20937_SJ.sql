...
PRINT '.........CREATE/MODIFY ttdStagingCompanyPartners.............' --Your Table Here
IF EXISTS (SELECT TOP 1 1 FROM sys.tables
			WHERE Name = 'ttdStagingCompanyPartners' --Your Table Here
			AND Type = 'U')
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'ScreeningProfile' --NEW column here
				AND Object_ID = Object_ID('ttdStagingCompanyPartners')) --Your Table Here
	BEGIN
		PRINT ''
	...
	ELSE
	BEGIN
		if exists (select TOP 1 1 from sys.tables where Name = 'tmp_ttdStagingCompanyPartners' --"tmp" + Your Table Here
					AND Type = 'U')
		begin
			DROP TABLE tmp_ttdStagingCompanyPartners --"tmp" + Your Table Here
		end
		
		CREATE TABLE [dbo].[tmp_ttdStagingCompanyPartners] --"tmp" + Your Table Here
		(
		...
		IF (SELECT TOP 1 1 FROM [ttdStagingCompanyPartners]) = 1 --Your Table Here
		BEGIN
		...
		BEGIN
			...
			EXEC usp_DBACopyTableIndexes ''
				,'ttdStagingCompanyPartners' --Your Table Here
		
			PRINT ''
			PRINT 'DROP TABLE..........'
			DROP TABLE [ttdStagingCompanyPartners] --Your Table Here
		
			PRINT ''
			PRINT 'RENAME TABLE....'
			
			EXECUTE sp_rename N'Tmp_ttdStagingCompanyPartners', N'ttdStagingCompanyPartners', 'OBJECT' --Your Table Here

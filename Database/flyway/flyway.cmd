...
set authpath="%INSTALLDIR%\native\auth\x86"

%JAVA_CMD% -d64 -version >nul 2>&1
if errorlevel 1 goto maybe32bit
set authpath="%INSTALLDIR%\native\auth\x64"
echo Java is 64 bit 
goto RUN
:maybe32bit
%JAVA_CMD% -d32 >nul 2>&1
...
%JAVA_CMD% -Djava.library.path=%authpath% %JAVA_ARGS% -cp "%CLASSPATH%;%INSTALLDIR%\lib\*;%INSTALLDIR%\lib\%FLYWAY_EDITION%\*;%INSTALLDIR%\drivers\*" org.flywaydb.commandline.Main %*

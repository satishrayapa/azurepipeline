...
IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'RegGroupIDList' --your column here
			AND Object_ID = OBJECT_ID('txdDPSSearchLogHist') AND MAX_LENGTH < 2000) --Your Table Here
BEGIN
	...
	EXEC usp_DBACopyTableIndexesByColumn '','txdDPSSearchLogHist','RegGroupIDList','VARCHAR',1,2000

	ALTER TABLE txdDPSSearchLogHist --Your Table Here
	ALTER COLUMN RegGroupIDList [VARCHAR] (2000) NOT NULL --your column here

	--Do not change 1st paramter.  
	EXEC usp_DBACreateTableIndexes '','txdDPSSearchLogHist' --Your Table Here
END
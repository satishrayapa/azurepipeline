...
IF EXISTS (SELECT TOP 1 1 FROM sys.columns WHERE name = 'Source' --your column here
			AND Object_ID = OBJECT_ID('txdDTSBatchQueue')) --Your Table Here
BEGIN
	...
	EXEC usp_DBACopyTableIndexesByColumn '','txdDTSBatchQueue','Source','VARCHAR',1,150

	ALTER TABLE txdDTSBatchQueue --Your Table Here
	ALTER COLUMN Source [VARCHAR] (150) NOT NULL --your column here

	--Do not change 1st paramter.  
	EXEC usp_DBACreateTableIndexes '','txdDTSBatchQueue' --Your Table Here
END